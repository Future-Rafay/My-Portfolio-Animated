...
const CustomMobileLink = ({ href, title, className = "", toggle }) => {
    ...
    return (
        ...
        >
            ...
            <span
                className={`h-[2px] inline-block bg-light absolute left-0 -bottom-0.5 group-hover:w-full transition-[width] ease-linear duration-300 dark:bg-dark ${router.asPath === href ? "w-full" : "w-0"
                    }`}
            >
                &nbsp;
            </span>
...
const Navbar = () => {
    ...
    return (
        <header className="w-full flex items-center justify-between px-32 py-8 font-medium dark:text-light relative z-10 lg:px-16 md:px-12 sm:px-8">
            ...
            >
                <span
                    className={`bg-dark dark:bg-light transition-all ease-out duration-300 block h-0.5 w-6 rounded-sm ${isOpen ? "rotate-45 translate-y-1" : "-translate-y-0.5"
                        }`}
                ></span>
                <span
                    className={`bg-dark dark:bg-light transition-all ease-out duration-300 block h-0.5 w-6 rounded-sm my-0.5 ${isOpen ? "opacity-0" : "opacity-100"
                        }`}
                ></span>
                <span
                    className={`bg-dark dark:bg-light transition-all ease-out duration-300 block h-0.5 w-6 rounded-sm ${isOpen ? "-rotate-45 -translate-y-1" : "translate-y-0.5"
            ...
            {isOpen ? (
                ...
                className="min-w-[70vw] flex flex-col justify-between items-center fixed top-1/2 left-1/2 z-30 -translate-x-1/2 -translate-y-1/2 bg-dark/90 text-light dark:bg-light/75 dark:text-dark rounded-lg backdrop-blur-md py-32">
                    ...
                    <nav className="flex items-center justify-center flex-wrap mt-4">
                        ...
                        <button
                            onClick={() => setMode(mode === "light" ? "dark" : "light")}
                            className={`ml-3 sm:ml-2 flex items-center justify-center rounded-full p-1  ${mode === "light" ? "bg-dark text-light" : "bg-light text-dark"
                                } `}
                        >
                            {mode === "dark" ? (
                                <SunIcon className="fill-dark" />
